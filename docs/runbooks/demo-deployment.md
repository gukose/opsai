...
4. Create a second Railway service for UniMock. Set config path `/railway.unimock.json`, do not create a public domain, provide DB variables, and use `SPRING_PROFILES_ACTIVE=prod`. Set backend `OPS_AI_UNIMOCK_BASE_URL` to its private hostname.
5. Deploy the backend manually. Flyway applies forward-only migrations, then DEMO bootstrap runs idempotently. Verify successful Flyway logs without credential values.
