...
class MasterDataAdminController(private val service:MasterDataAdminService,private val current:CurrentUserContextResolver){
 ...
 /** Read-only operational projection used by task creation; it is not room administration. */
 @GetMapping("/hotels/{hotelId}/rooms/lookup") @PreAuthorize("@permissionGuard.hasHotelPermission(#hotelId, 'TASK_READ')") fun roomLookup(@PathVariable hotelId:UUID,@RequestParam(required=false) q:String?,@RequestParam(required=false) active:Boolean?,@RequestParam(defaultValue="0") page:Int,@RequestParam(defaultValue="200") size:Int)=service.rooms(hotelId,q,null,null,null,active,page,size)
